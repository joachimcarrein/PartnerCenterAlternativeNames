...
- **`push` is scoped to `branches: [main]` on purpose.** Unscoped, a feature-branch push runs the suite a second time alongside the PR run, producing two check runs both named `test`; both match the required check, and the last to report decides a split result. It also means a workflow opening PRs with `GITHUB_TOKEN` can never work here (GitHub suppresses `pull_request` for those, so the PR would have no `test` check at all) — an accepted trade-off, PRs are opened by hand.
...
- **`main` is protected and nothing lands on it directly.** A repository ruleset blocks force pushes and deletions, and requires a pull request with the `test` check passing (0 approvals — a solo maintainer cannot approve their own PR, so requiring one would deadlock the repo). Branch first, push the branch, open the PR by hand, merge with **Squash and merge** to keep `main` linear. The bypass list is empty, so this applies to the owner too; the escape hatch is flipping the ruleset to *Disabled* in Settings → Rules, not a force push.
